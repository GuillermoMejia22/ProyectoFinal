# -*- coding: utf-8 -*-
"""
Created on Wed Apr 28 20:13:59 2021
...
def relaciones(tab, dominios, rela):
    ...
    while r<len(tab):
        ...
        try:
            ...
            #####patron pnounphrases#####            
            elif tab[r]['eti']=='NPHRASE' or tab[r]['eti']=='NOUN':
                r+=1
...
sal=open('etiquetas_propuesta_nlp_medicam.txt','w', encoding='utf-8')
sal1=open("relaciones_propuestas_nlp_medicam.txt",'w', encoding='utf-8')

document=open("datos_medicam.txt","r", encoding="utf-8")
...
while k <=len(lins):
    ...
    if ".000." in lins[k]:
        ...
        if "Contraindicaciones : " in lins[k]:
            ...
            la=aux_con[1].replace('(','').replace('(','').replace(',',' ,').replace(";"," ;").replace(':',' :').replace("\n","")
